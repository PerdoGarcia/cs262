...
def instruction_performer(message_queue, port_number, clock_speed, run_event, connections, connections_lock, run_mesages):
    ...
    print(f"DEBUG: Machine {machine_id} established connections to: {list(connections.keys())}")
    run_mesages.wait()
    ...
    while run_event.is_set():
        ...
        for _ in range(clock_speed):
            print(f"DEBUG: Machine {machine_id} checking queue, size: {message_queue.qsize()}")
            if not message_queue.empty():
                ...
                if event == 1:
                    if connections:
                        ...
                        log_message(log_file, "  SEND  ", f"Sent to machine {recipient_id}",  message["time"], message_queue.qsize())
                    ...
                    for recipient, sock in connections.items():
                        if recipient != machine_id:
                            ...
                            log_message(log_file, "  SEND  ", f"Sent to machine {recipient}",  message["time"], message_queue.qsize())
...
def main():
    ...
    try:
        ...
        m2.join()
        m3.join()
        print("Clocks successfully closed")
